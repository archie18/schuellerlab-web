...
def get_res_asa(asafile):
    ...
    for res in model.get_residues():
        ...
        try:
            res_asa[resname] = sum([atom.get_bfactor() for atom in res.get_atoms()]) # BioPython version > 1.69
        except AttributeError:
            res_asa[resname] = sum([atom.get_bfactor() for atom in res.get_atom()])  # BioPython version <= 1.69
